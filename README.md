...
| 匿名访问 | 支持通过配置开启或关闭匿名客户端连接 |
| ClientID 策略 | 拒绝新连接 / 断开旧连接 两种模式可选 |
| 消息持久化 | H2 嵌入式数据库，AES 文件级加密存储 |
| 消息桥接 | 内置 Bridge 引擎，支持多 Broker 消息转发与路由规则 |
| 动态设备认证 | 支持基于 H2 数据库的客户端凭证（用户名/密码）动态校验，密码以 BCrypt 加密存储 |
| 动态 ACL 鉴权 | 支持基于数据库的发布/订阅权限控制，内置 ACL 缓存提升高频匹配效率 |
| 动态变量占位符 | 支持在 ACL 主题过滤器中使用 `${clientId}` 和 `${username}` 占位符进行按设备动态权限隔离 |
| 保留消息 (Retained) | 内置保留消息反射检索和一键清除功能，支持管理终端实时管控 |

### 📊 Web 管理控制台

- **仪表盘** — JVM 内存/线程/GC、CPU、磁盘等系统指标实时总览
- **MQTT 监控** — WebSocket 实时推送连接数、消息速率、订阅统计、在线客户端
- **客户端管理** — 在线客户端详情查看、强制断开
- **安全管理** — 可视化管理设备凭证及 ACL 访问规则（支持横向响应式布局、单选及下拉过滤、无冗余暂无数据提示）
- **消息记录** — 按 Topic/客户端/时间范围检索历史消息，支持趋势统计图
- **MQTT 调试工具** — 浏览器内嵌 MQTT 客户端，支持订阅/发布/QoS/Retain/延迟消息
- **日志管理** — SSE 实时日志流（节流防抖合并渲染、终端风格、高亮着色），支持**运行时动态调整** ROOT/Broker/Web 日志级别
- **保留消息** — 系统当前缓存的保留消息列表展示与可视化清除
...
| `GET` | `/v1/monitor/retained` | 获取 Broker 当前所有保留消息 |
| `DELETE` | `/v1/monitor/retained` | 清除特定主题的保留消息 |
| `POST` | `/v1/monitor/publish` | 通过 Broker 发布消息 |
| `GET` | `/v1/messages` | 分页查询历史消息 |
| `GET` | `/v1/messages/stats` | 消息统计概览 |
| `GET` | `/v1/messages/trend` | 消息趋势数据 |
| `GET` | `/v1/logs/level` | 实时获取系统及组件当前的日志级别 |
| `POST` | `/v1/logs/level` | 动态修改指定的日志记录器级别 |
| `GET` | `/v1/logs/sse` | 实时日志流 (SSE) |
| `GET`/`POST`/`PUT`/`DELETE` | `/v1/client/credentials` | 设备凭证的 CRUD 管理接口 |
| `GET`/`POST`/`PUT`/`DELETE` | `/v1/client/acl` | ACL 规则的 CRUD 管理接口 |
| `WS` | `/ws/monitor` | WebSocket 实时监控 |

## 设备动态安全配置

系统支持通过 Web 界面或 REST API 动态热配置设备凭证（设备用户名及密码）与发布/订阅的 ACL 过滤规则。

### 1. 动态设备凭证
- **密码存储安全**：用户设定的设备密码在保存入库时会通过 `BCrypt` 单向哈希算法进行强加密存储，在数据库层面彻底对管理员不可读，防止敏感信息泄漏。
- **配置即时生效**：设备凭证变更时系统会自动清除设备连接认证缓存，新旧配置即时平滑衔接。

### 2. ACL 访问控制规则
- **规则优先级**：ACL 过滤时由优先级数值（`Priority`）控制匹配顺序，数值越大越先匹配；且当动作设为 `ALLOW`/`DENY` 时可灵活实现动态授权或黑名单阻断。
- **动态占位符支持**：支持主题过滤器中使用 `${clientId}` 和 `${username}` 占位符。
  * 规则示例：
    - 主题过滤器：`device/${clientId}/telemetry`
    - 权限：`PUB`（仅发布）
    - 动作：`ALLOW`（允许）
  * 作用效果：允许任何设备发布数据到以其自身客户端 ID 命名的主题（如 `device/sensor-01/telemetry`），并同时自动防御和阻止其越权发布其他设备的主题。
